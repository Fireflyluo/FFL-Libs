/*-----------------------------------------------File Info------------------------------------------------
** File Name:               adhoc_crc8.c
** Created date:            2026.5.14
** author:                  Fireflyluo
** Version:                 V0.1
** Descriptions:            CRC8 校验实现
**                          固定参数 poly=0x07, init=0x00, xorout=0x00 的 CRC8 计算与帧校验
**--------------------------------------------------------------------------------------------------------
*/

#include "adhoc_crc8.h"

static ADHOC_CRC8_TABLE_STORAGE const uint8_t s_adhoc_crc8_table[256] = {
...
/**
 * @brief 计算任意数据的 CRC8
 * @param data 数据指针(NULL时返回 init^xorout)
 * @param len  数据长度(字节)
 * @return CRC8 值
 */
uint8_t adhoc_crc8_compute(const uint8_t *data, uint16_t len)
{
    ...
    {
        crc = s_adhoc_crc8_table[(uint8_t)(crc ^ data[index])];
    }
...
/**
 * @brief 计算32B帧的 CRC8(覆盖前31字节, 不含 CRC 字段本身)
 */
