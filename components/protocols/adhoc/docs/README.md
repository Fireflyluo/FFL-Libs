...
- `ffl.adhoc.test` 和 `ffl.adhoc.cxx-test` 只覆盖 GCC host 上的纯函数编译与运行 smoke，包括 CRC8、32B frame 编解码和公开头的 C++ 调用边界。
- Host smoke 不等价于射频链路、时序、电源、串口或目标板验证；它不证明无线收发、ACK、入网和硬件端口适配正确。
- 链路硬件验证必须在明确的目标板、射频端口和日志条件下单独执行，并记录实际链路证据。
