...
static void ffl_sw_timer_wheel_insert(ffl_sw_timer_t *timer, uint32_t ticks) {
  ...
  timer->slot =
      (uint16_t)((g_current_slot + slot_offset) % FFL_SW_TIMER_WHEEL_SIZE);
...
int ffl_sw_timer_start(ffl_sw_timer_t *timer, uint32_t delay_ms,
  ...
  timer->period_ticks =
      (period_ms == 0U) ? 0U : ffl_sw_timer_ms_to_ticks(period_ms);
...
void ffl_sw_timer_process(void) {
  ...
  while (local_list != NULL) {
    ...
    if ((node->periodic != 0U) && (node->rearmed == 0U)) {
      ffl_sw_timer_enter_critical();
