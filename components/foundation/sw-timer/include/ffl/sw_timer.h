 ...
 *
 * `ffl_sw_timer_tick_isr()` 应由系统 tick 中断调用，`ffl_sw_timer_process()`
 * 应在主循环或 任务上下文调用。组件不绑定 MCU、RTOS
 * 或具体临界区实现，平台保护通过钩子注入。
 */
...
typedef struct ffl_sw_timer {
  ...
  uint8_t processing;         /**< 到期回调正在任务上下文执行 */
  uint8_t rearmed;            /**< 回调期间已由 ffl_sw_timer_start() 重新调度 */
 ...
 *
 * @note
 * 若未设置，软件定时器在单线程场景下可正常工作；若在中断/多线程环境下使用，
 ...
 *         - -2: 未初始化(g_tick_ms为0)
 *
 * @note 如果定时器已经在运行，会先停止再重新启动。周期回调中重启自身时，
 *       新设置会替代自动周期重装。
 */
int ffl_sw_timer_start(ffl_sw_timer_t *timer, uint32_t delay_ms,
                       uint32_t period_ms, ffl_sw_timer_expired_fn expired_fn,
                       void *arg);
 ...
 *
 * @note 如果定时器未激活或为NULL，函数直接返回。周期回调中停止自身会取消
 *       本轮回调结束后的自动周期重装。
 */
