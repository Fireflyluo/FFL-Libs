...
/* 6x8，列宽 6（含 1px 右留白）。bit0=最左。仅常用 ASCII。 */
static const uint8_t s_f32 = 0x00;
...
static const uint8_t s_font[95][8] = {
    ...
    {0x00, 0x20, 0x00, 0x00, 0x00, 0x20, 0x20, 0x40}, /* ; */
...
const uint8_t *font6x8_glyph(char ch) {
  uint8_t idx = (uint8_t)(unsigned char)ch;
  (void)s_f32;
  if (idx < 32u || idx > 126u) {
    idx = (uint8_t)'?';
  }
  return s_font[idx - 32u];
}
