...
/* 私有宏定义 */
#define QMI8658A_I2C_ADDR    0x6A  // 默认 I2C 地址（SA0=0 时）

/**
  * @brief  I2C 读取寄存器（用户需要根据自己的平台实现）
  * @param  handle: 设备句柄（用户自定义的 I2C 句柄）
  * @param  reg: 寄存器地址
  * @param  data: 读取数据缓冲区
  * @param  len: 读取数据长度
  * @retval 0: 成功；非 0: 失败
...
int32_t qmi8658a_soft_reset(qmi8658a_ctx_t *ctx)
{
    ...
    /* 等待复位完成 */
    HAL_Delay(15); // 复位时间约 15ms
    
    return ret;
}

/**
  * @brief  读取设备 ID（验证设备连接）
  * @param  ctx: 驱动上下文指针
  * @param  buff: 存储设备 ID 的缓冲区（至少 1 字节）
  * @retval 0: 成功；非 0: 失败
  */
...
/**
  ...
  * @param  ctx: 驱动上下文指针
  * @param  val: 目标 ODR（qmi8658a_accel_odr_t 枚举值）
  * @retval 0: 成功；非 0: 失败
  */
...
/**
  ...
  * @param  ctx: 驱动上下文指针
  * @param  val: 存储当前 ODR 的变量（qmi8658a_accel_odr_t 指针）
  * @retval 0: 成功；非 0: 失败
  */
...
/**
  ...
  * @param  ctx: 驱动上下文指针
  * @param  val: 目标量程（qmi8658a_accel_fs_t 枚举值）
  * @retval 0: 成功；非 0: 失败
  */
...
/**
  ...
  * @param  ctx: 驱动上下文指针
  * @param  val: 存储当前量程的变量（qmi8658a_accel_fs_t 指针）
  * @retval 0: 成功；非 0: 失败
  */
...
/**
  ...
  * @param  ctx: 驱动上下文指针
  * @param  val: 目标 ODR（qmi8658a_gyro_odr_t 枚举值）
  * @retval 0: 成功；非 0: 失败
  */
...
/**
  ...
  * @param  ctx: 驱动上下文指针
  * @param  val: 存储当前 ODR 的变量（qmi8658a_gyro_odr_t 指针）
  * @retval 0: 成功；非 0: 失败
  */
...
/**
  ...
  * @param  ctx: 驱动上下文指针
  * @param  val: 目标量程（qmi8658a_gyro_fs_t 枚举值）
  * @retval 0: 成功；非 0: 失败
  */
...
/**
  ...
  * @param  ctx: 驱动上下文指针
  * @param  val: 存储当前量程的变量（qmi8658a_gyro_fs_t 指针）
  * @retval 0: 成功；非 0: 失败
  */
...
/**
  * @brief  读取时间戳值
  * @param  ctx: 设备上下文指针
  * @param  timestamp: 存储时间戳的指针（24 位值）
  * @retval 0: 成功；非 0: 失败
  */
...
/**
  * @brief  读取原始加速度计数据（6 字节，X/Y/Z 各 2 字节）
  * @param  ctx: 驱动上下文指针
  * @param  buff: 存储加速度数据的缓冲区（至少 6 字节）
  * @retval 0: 成功；非 0: 失败
  */
...
/**
  * @brief  读取原始陀螺仪数据（6 字节，X/Y/Z 各 2 字节）
  * @param  ctx: 驱动上下文指针
  * @param  buff: 存储陀螺仪数据的缓冲区（至少 6 字节）
  * @retval 0: 成功；非 0: 失败
  */
...
int32_t qmi8658a_fifo_read(qmi8658a_ctx_t *ctx, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    /* 组合 10 位样本计数 (FIFO_STATUS[1:0] + FIFO_SMPL_CNT) */
    sample_count = ((fifo_status & 0x03) << 8) | fifo_smpl_cnt;
    
    /* 步骤 2：计算需要读取的字节数 */
    bytes_to_read = sample_count * 2;  // 每个样本 2 字节
