...
static int sc7a20_core_lock_cas(volatile uint8_t *ptr, uint8_t expected, uint8_t desired)
{
#if defined(_MSC_VER)
...
static void sc7a20_core_lock_release(volatile uint8_t *ptr)
{
#if defined(_MSC_VER)
...
int sc7a20_core_try_lock(sc7a20_dev_t *dev)
{
    ...
    if (!sc7a20_core_lock_cas(&dev->in_use, 0u, 1u)) {
        return -EBUSY;
    }
    return 0;
}
...
{
    if (dev != NULL) {
        sc7a20_core_lock_release(&dev->in_use);
    }
}
