...
static int sht40_core_lock_cas(volatile uint8_t *ptr, uint8_t expected, uint8_t desired)
{
#if defined(_MSC_VER)
...
static void sht40_core_lock_release(volatile uint8_t *ptr)
{
#if defined(_MSC_VER)
...
int sht40_core_try_lock(sht40_dev_t *dev)
{
    ...
    if (!sht40_core_lock_cas(&dev->in_use, 0u, 1u)) {
        return -EBUSY;
    }
    return 0;
}
...
{
    if (dev != 0) {
        sht40_core_lock_release(&dev->in_use);
    }
}
